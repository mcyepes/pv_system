...
def main():
    ...
    for G_input in G_range:
        for T_input in T_range:
            # Calcular el modelo PV
            resultados, Vmpp, Impp, P_max = pv.modelo_pv(G=G_input, T=T_input)
            #print(resultados.head())
            print(f"G = {G_input}, T = {T_input}, Vmp = {Vmpp:.2f} V, Imp = {Impp:.2f} A, Pmax = {P_max:.2f} W")
            pv_system.append([G_input, T_input, Vmpp, Impp, P_max])
    print('Calculations Finish')
    # Conexion a la base de datos
    conn = sqlite3.connect("pv_system.db") # Si no se tiene la base de datos, se crea
    c = conn.cursor() # Se crea un cursor para ejecutar las sentencias SQL

    # Crear una tabla si no existe
    c.execute('''CREATE TABLE IF NOT EXISTS pv_system (
    ...
    c.executemany('INSERT INTO pv_system (irradiancia, temperature, vpm, ipm, pmax) VALUES (?, ?, ?, ?, ?)', pv_system)
    conn.commit() # Guardar los cambios en la base de datos
    conn.close() # Cerrar la conexión a la base de datos
    print("PV System db creado con exito")
